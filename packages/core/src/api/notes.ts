...
// Delete a note. 'system' goes through the platform OS-trash hook when one
...
// back navigation and sidebars that clear the selection.
export const closeNote = () => {
	appState.activeFile = null;
};

// Open the version-history view for a note: open the note first (the
// history panel reads the active file), then switch the detail sidebar to
// the history tab.
export const openNoteHistory = async (path: string) => {
...
// first, so restore is itself undoable. Listeners are notified so history
// views refresh, and the editor reloads if the restored note is open.
