...
type SidebarTarget = 'page' | 'note-detail' | 'detail';

// Drag-resize action for the sidebars. 'page' is the left sidebar which grows
// to the right, 'note-detail' (alias 'detail') is the right sidebar which
// grows to the left.
export const sidebarResize: Action<HTMLElement, SidebarTarget> = (node, target) => {
	const isPageSidebar = target !== 'note-detail' && target !== 'detail';
