<script lang="ts">
	...
	import { getEditorSelectionText, goToSearchResult } from '@tactile/core/utils/editor';
	...
	const withEditor = (fn: (ed: NonNullable<typeof editor>) => void) => {
		if (editor) fn(editor);
	};
...
>
	<Shortcut
		...
		callback={() => {
			if (appState.editorSearchActive) {
				...
				appState.editorSearchValue = editor ? getEditorSelectionText(editor) : '';
	...
	<Shortcut
		...
		callback={() => {
			if (appState.editorSearchActive) {
				withEditor((ed) => {
					ed.commands.previousSearchResult();
					goToSearchResult(ed);
				});
			}
		}}
