<script lang="ts">
	import { setSourceContent } from '../../../api/editor';
...
<div class={cn('w-full h-[calc(100%-97px)]', isMobile() ? 'px-5' : 'px-8')}>
	<div class="flex w-fit max-w-[655px] h-full mx-auto min-w-0">
		...
		<textarea
			bind:this={textarea}
			value={appState.sourceContent}
			oninput={(e) => {
				setSourceContent(e.currentTarget.value);
				scheduleSave();
			}}
