...
fn extract_context(content: &str, index: usize, query_len: usize) -> String {
    ...
    let end = content[index + query_len..]
        .find("\n\n")
        .map_or(content.len(), |i| index + query_len + i);

    let context = &content[start..end];
    let mut sentences: Vec<&str> = context
        .split(|c| c == '.' || c == '?' || c == '!')
        .collect();

    if sentences.len() > 3 {
        let query_sentence_index = sentences
            .iter()
            .position(|&s| s.contains(&content[index..index + query_len]))
            .unwrap_or(0);
        ...
        if let Ok(entries) = fs::read_dir(path) {
            for entry in entries.flatten() {
                ...
                if entry_path.is_dir() && recursive {
                    file_paths.extend(read_path_files(
                    ...
                    && entry_path
                        .extension()
                        .map_or(false, |ext| ext == &extension[1..])
                {
