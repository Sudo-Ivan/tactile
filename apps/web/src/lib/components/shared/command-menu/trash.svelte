<script lang="ts">
	...
	import { deleteTrashItem, emptyTrash, listTrash, restoreFromTrash } from '@/api/trash';
	...
	let { onPageChange: _onPageChange }: Props = $props();
	...
	function arm(key: string): boolean {
		...
		pending = key;
		clearTimeout(pendingTimer);
		pendingTimer = setTimeout(() => (pending = null), 4000);
		return false;
	}
					...
					>
						...
						<div class="flex w-full items-center justify-between gap-2">
							<span class="truncate text-foreground/80">
								{item.originalPath.split('/').filter(Boolean).slice(1).join(' > ')}
							</span>
